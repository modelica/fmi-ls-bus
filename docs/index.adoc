...
image::images/architecture.svg[width=80%, align="center"]
...
The content of the binary variable is any number of frames, including none.
One frame is composed according to the following table.
...
The OP code contains an operation byte and encodes the size of the OP specific content field length and the repeat-bit (most significant bit of the second byte).
...
Such a clock is scheduled by the sender to indicate the transmission of the corresponding frame or frames.
The output clock type, periodic or aperiodic, is defined by the sending FMU.
The input clocks shall be triggered clocks.
The importer will then connect and merge clock activations, even those of different clock types, as required by the network semantics.

_[Even if FMUs connected to the same framework use output clocks of different types, the importer can aggregate these events and forward them to the corresponding triggered input clocks.]_
...
image::images/bus_simulation_example.svg[width=80%, align="center"]
...
image::images/bus_terminal.svg[width=80%, align="center"]
