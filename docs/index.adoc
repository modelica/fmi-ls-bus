...
The attribute `name` of the `<Terminal>` must match the root name of its <<NetworkDescriptionFile>> [<<rule_1, rule 1>>][[apply_rule_1]].
_[e.g. `Powertrain`, if the file is `/extra/org.modelica.fmi.layered_BUS/Powertrain.dbc`]_.

Attribute definitions [<<rule_2, rule 2>>][[apply_rule_2]]::
...
Element definitions [<<rule_3, rule 3>>][[apply_rule_3]]::
