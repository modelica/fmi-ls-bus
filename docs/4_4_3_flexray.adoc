...
Within a defined time window, multiple <<low-cut-flexray-transmit-operation, `Transmit`>> operations for the same logical signal (same slot/cycle) may be transmitted. In such cases, the Last-Is-Best principle applies: A Network FMU may update a <<low-cut-flexray-transmit-operation, `Transmit`>> operation as long as the same value is used for the `Slot ID` argument within the defined time window. Only the last <<low-cut-flexray-transmit-operation, `Transmit`>> operation within the same time window shall be considered valid. All preceding <<low-cut-flexray-transmit-operation, `Transmit`>> operations shall be treated overwritten and must not be processed.
This principle applies consistently to both communication via bus simulation and direct FMU-to-FMU communication.

* If a Bus Simulation is involved, the Bus Simulation shall ensure that only the valid (the last) <<low-cut-flexray-transmit-operation, `Transmit`>> operation within the time window is forwarded to the receiving FMU. Overwritten <<low-cut-flexray-transmit-operation, `Transmit`>> operations according to the Last-Is-Best principle shall not be forwarded.

* In the case of direct FMU-to-FMU communication, the receiving FMU shall be responsible for enforcing the Last-Is-Best principle. The FMU will receive all <<low-cut-flexray-transmit-operation, `Transmit`>> operations provided by the sending FMU within the same slot/cycle. The receiving FMU shall ensure that only the last <<low-cut-flexray-transmit-operation, `Transmit`>> operation within the time window is considered valid and that all preceding <<low-cut-flexray-transmit-operation, `Transmit`>> operations shall be discarded and must not be processed.

The <<low-cut-flexray-cancel-operation, `Cancel`>> operation allows the cancellation of such buffered <<low-cut-flexray-transmit-operation, `Transmit`>> operations.
...
In concrete terms, this means that a Network FMU must provide the intended <<low-cut-flexray-transmit-operation, `Transmit`>> operation before the simulation reaches the slot’s action point (see ActionPoint Offset argument within the Configuration operation) via <<low-cut-tx-clocks, Tx Clocks>>.
...
For a static segment, a Network FMU may provide multiple <<low-cut-flexray-transmit-operation, `Transmit`>> operations for the same slot/cycle during the simulation. The time window where the Last-Is-Best principle applies is defined in this section.
The operation provided closest to the slot’s action point (see ActionPoint Offset argument within the Configuration operation) shall be used. Earlier operations for the same slot/cycle may be overridden by later ones. <<low-cut-flexray-transmit-operation, Transmit>> operations that are provided after the slot’s action point shall not be considered for the current time window and shall instead be used for the same slot and cycle in the next cycle iteration.
...
In the case of direct FMU-to-FMU communication, the receiving FMU is responsible for processing the <<low-cut-flexray-transmit-operation, Transmit>> operation closest to the slot’s action point at the time defined by the <<low-cut-flexray-delivery-on-boundary-parameter, `DeliveryOnBoundary`>> configuration parameter.

.Bus Communication Points within static segment.
[#figure-flexray-static-segment-bus-communication-points]
image::images/flexray_static_segment_bus_communication_points.svg[width=700]
...
* stem:[t \in T_{\mathrm{Valid}_{\mathrm{Static}}}
    \iff
    T_{\mathrm{AP}}(i_{\mathrm{iteration}} - 1, i_{\mathrm{Cycle}}, i_{\mathrm{Slot}})
    < t \le
    T_{\mathrm{AP}}(i_{\mathrm{iteration}}, i_{\mathrm{Cycle}}, i_{\mathrm{Slot}})].

A Transmit operation is valid if it is provided at a time stem:[t] within the interval spanning from immediately after the action point of slot stem:[i_{\mathrm{Slot}}] in stem:[i_{\mathrm{Cycle}}] of iteration stem:[i_{\mathrm{iteration}} - 1] to the action point of the same slot and cycle in iteration stem:[i_{\mathrm{iteration}}].
...
As in the static segment, the Last-Is-Best principle applies and <<low-cut-flexray-transmit-operation, `Transmit`>> operations are allowed from a `Minislot ActionPoint` (see <<low-cut-flexray-configuration-operation, `Configuration`>> operation) to the same point in the next cycle iteration. The <<low-cut-flexray-transmit-operation, `Transmit`>> operation closest to the slot’s minislot action point is used.
...
If no Bus Simulation is involved, the receiving FMU shall ensure that incoming <<low-cut-flexray-transmit-operation, `Transmit`>> operations are processed in the correct minislot, considering the transmission duration and the number of remaining minislots within the current cycle. Transmit operations whose transmission duration exceeds the number of remaining minislots in the current cycle shall be deferred and processed in the same slot and cycle in the next cycle iteration.
