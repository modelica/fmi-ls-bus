...
If multiple <<low-cut-flexray-symbol-operation, `Symbol`>> operations of type `COLLISION_AVOIDANCE_SYMBOL` are received at the same time, the Bus Simulation shall solve this conflict by redirecting the <<low-cut-flexray-symbol-operation, `Symbol`>> operation with the smallest `Slot ID` argument value and discard all other <<low-cut-flexray-symbol-operation, `Symbol`>> operations.
If no Bus Simulation is involved, the Network FMUs shall solve this conflict by using the <<low-cut-flexray-symbol-operation, `Symbol`>> operation with the smallest `Slot ID` argument value and ignore all other <<low-cut-flexray-symbol-operation, `Symbol`>> operations.
