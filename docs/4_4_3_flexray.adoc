...
In concrete terms, this means that a Network FMU must provide the intended <<low-cut-flexray-transmit-operation, `Transmit`>> operation before the simulation reach the slot’s action point (see ActionPoint Offset argument within the Configuration operation) via <<low-cut-tx-clocks, Tx Clocks>>.
...
For a static segment, a Network FMU may provide multiple Transmit operations for the same slot/cycle during the simulation.
The Bus Simulation shall apply the Last-Is-Best principle: the operation provided closest to the slot’s action point (see ActionPoint Offset argument within the Configuration operation) shall be used for transmission. Earlier operations for the same slot/cycle may be overridden by later ones.
...
As in the static segment, the Last-Is-Best principle is applied and the permitted interval to provide <<low-cut-flexray-transmit-operation, `Transmit`>> operations is defined between the `Minislot ActionPoint Offset` (see <<low-cut-flexray-configuration-operation, `Configuration`>> operation) and the `Minislot ActionPoint Offset` (see <<low-cut-flexray-configuration-operation, `Configuration`>> operation) of the same minislot/cycle of the next cycle iteration.
