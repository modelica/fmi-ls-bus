...
The time window where the Last-Is-Best principle applies is defined in this section.
...
Earlier operations for the same slot/cycle may be overridden by later ones.
...
In the case of directly connected Network FMUs, the receiving FMU is responsible for processing the <<low-cut-flexray-transmit-operation, `Transmit`>> operation closest to the slot’s action point at the time defined by the <<low-cut-flexray-delivery-on-boundary-parameter, `DeliveryOnBoundary`>> configuration parameter.
