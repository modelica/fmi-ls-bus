...
The configuration parameter <<low-cut-flexray-delivery-on-boundary-parameter, `DeliveryOnBoundary`>> was introduced within a FlexRay Bus Terminal to switch the behavior in the Bus Simulation.
...
If a Network FMU is supporting the delivery on slot boundaries only, the configuration parameter shall be omitted.
...
----
 ...
 Description:   "Specifies whether the respective Network FMU expects Transmit, 
                Confirm- and Bus Error operations on a concrete slot boundary."
 Type:          Boolean
 Causality:     parameter
 Variability:   fixed
 Start:         "true"
