...
This principle applies consistently to both communication via bus simulation and direct communication.

* If a Bus Simulation is involved, the Bus Simulation shall ensure that only the last <<low-cut-flexray-transmit-operation, `Transmit`>> operation within the time window is forwarded to the receiving FMU.

* In the case of direct communication, the receiving FMU shall ensure that only the last <<low-cut-flexray-transmit-operation, `Transmit`>> operation is processed.
...
The operation provided closest to the slot’s action point (see ActionPoint Offset argument within the Configuration operation) shall be used.
...
In the case of direct communication, the receiving FMU is responsible for processing the <<low-cut-flexray-transmit-operation, `Transmit`>> operation closest to the slot’s action point at the time defined by the <<low-cut-flexray-delivery-on-boundary-parameter, `DeliveryOnBoundary`>> configuration parameter.
...
For direct communication, the Last-Is-Best principle shall be enforced by the receiving FMU and the <<low-cut-flexray-delivery-on-boundary-parameter, `DeliveryOnBoundary`>> configuration parameter must be taken into account in order to process the <<low-cut-flexray-transmit-operation, `Transmit`>> operation at the correct time.
