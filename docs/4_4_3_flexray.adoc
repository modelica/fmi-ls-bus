...
[stem] 
++++
    T_{Tx_{Static}}(tc, i_{Cycle}, i_{StaticSlot}) = T_{CycleStart}(tc, i_{Cycle}) + (i_{StaticSlot} - 1) * L_{StaticSlot}
++++

, whereby:

* stem:[i_{StaticSlot}] represents the index of the static slot for transmission
...
[stem] 
++++
    T_{Valid} = [ T_{Tx_{Static}}(tc, i_{Cycle}, i_{StaticSlot}), T_{Tx_{Static}}(tc, i_{Cycle}, i_{StaticSlot}) + T_{ActionPoint_{Static}} ]
++++
...
Within a Network FMU the point in time when a specified <<low-cut-flexray-transmit-operation, `Transmit`>> operation shall be provided can be compute like:

[stem] 
++++
   T_{Tx_{Dynamic}}(tc, i_{Cycle}, i) = T_{Tx_{Static}}(tc, i_{Cycle}, N_{StaticSlot})
        + \sum_{j=N_{StaticSlot} + 1}^{i - 1} L_{DynamicSlot_j} * L_{Minislot}
++++
...
[stem] 
++++
    T_{Valid} = [ T_{Tx_{Dynamic}}(tc, i_{Cycle}, i), T_{Tx_{Dynamic}}(tc, i_{Cycle}, i) + T_{ActionPoint_{Dynamic}} ]
++++
