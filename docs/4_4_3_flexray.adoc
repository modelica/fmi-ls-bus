...
Overall, the following operations and groups of operations exist:
...
h|Description
3+|Initiates the transmission of FlexRay frames.
...
Depending on the simulation details, the Bus Simulation might respond with a <<low-cut-flexray-confirm-operation, `Confirm`>>, <<low-cut-flexray-bus-error-operation, `Bus Error`>> or <<low-cut-flexray-format-error-operation, `Format Error`>> operation.
...
h|Description
3+|Initiates the cancellation transmission of FlexRay frames within a Bus Simulation.
...
Depending on the type (qualified via `Error Flags` argument value) of <<low-cut-flexray-bus-error-operation, `Bus Error`>>, either only the <<low-cut-flexray-transmit-operation, `Transmit`>> producing or all Network FMUs must be notified via <<low-cut-flexray-bus-error-operation, `Bus Error`>> operation (see description column of <<table-flexray-error-code-values>>).
Only Network FMUs with the corresponding optionally exposed <<low-cut-flexray-bus-notification-parameter, `BusNotifications`>> parameter set to `fmi3True` might wait for this operation.
|====

The following values for `Error Flags` argument are specified:
...
The following values for `Segment Indicator` argument are specified:

.Overview of the available segment indicator values.
...
The following values for `Coldstart Node` argument are specified:
...
|[[table-flexray-bus-start-start-time-parameter]]
Start Time
|8 bytes
...
|Slot ID
|2 byte
|The Slot ID of the startup frame.
In case of `COLLISION_AVOIDANCE_SYMBOL`, the Slot ID of the startup frame to be sent must be specified.
...
If a Bus Simulation is involved (see <<common-concepts-composition-with-dedicated-bus-simulation-fmu>> and <<common-concepts-importer-with-integrated-bus-simulation>>), the FMU importer forwards the operation initially to the Bus Simulation.
If multiple <<low-cut-flexray-symbol-operation, `Symbol`>> operations of type `COLLISION_AVOIDANCE_SYMBOL` are received at the same time, the Bus Simulation may solve this conflict by redirecting the <<low-cut-flexray-symbol-operation, `Symbol`>> operation with the smallest `Slot ID` argument value and discard all other <<low-cut-flexray-symbol-operation, `Symbol`>> operations.
In any case and in the final instance, the Network FMUs shall solve this conflict by using the <<low-cut-flexray-symbol-operation, `Symbol`>> operation with the smallest `Slot ID` argument value and ignore all other <<low-cut-flexray-symbol-operation, `Symbol`>> operations.
...
The following values for `Type` argument are specified:
...
NOTE: The default value `false` allows a simple integration of Network FMUs to simulation scenarios where <<low-cut-flexray-confirm-operation, `Confirm`>> or <<low-cut-flexray-bus-error-operation, `Bus Error`>> operations are not used.
...
image::images/flexray_wake_up.svg[width=700]
...
image::images/flexray_startup.svg[width=750]
...
image::images/flexray_startup_coldstart_emulation.svg[width=750]
...
The <<low-cut-flexray-transmit-operation, `Transmit`>> operation represents the core of a bus transmission.
...
A <<low-cut-flexray-confirm-operation, `Confirm`>> and <<low-cut-flexray-bus-error-operation, `Bus Error`>> operation represents feedback from a Bus Simulation for a previously carried out <<low-cut-flexray-transmit-operation, `Transmit`>> operation.
Depending on the <<low-cut-flexray-bus-notification-parameter, `BusNotifications`>> parameter, for a Network FMU a successful transmission of a <<low-cut-flexray-transmit-operation, `Transmit`>> operation results in a <<low-cut-flexray-confirm-operation, `Confirm`>> operation, an unsuccessful <<low-cut-flexray-transmit-operation, `Transmit`>> operation in a <<low-cut-flexray-bus-error-operation, `Bus Error`>> operation.
...
Within a defined time window, multiple <<low-cut-flexray-transmit-operation, `Transmit`>> operations for the same logical signal (same slot/cycle) may be transmitted.
In such cases, the Last-Is-Best principle applies: A Network FMU may update a <<low-cut-flexray-transmit-operation, `Transmit`>> operation as long as the same value is used for the `Slot ID` argument within the defined time window.
On the receiver side, only the last <<low-cut-flexray-transmit-operation, `Transmit`>> operation within the time window shall be considered valid.
All preceding <<low-cut-flexray-transmit-operation, `Transmit`>> operations shall be treated overwritten and must not be processed.
This principle applies consistently to all three communication architectures (see <<common-concepts-system-compositions>>).

* If a Bus Simulation is involved (see <<common-concepts-composition-with-dedicated-bus-simulation-fmu>> and <<common-concepts-importer-with-integrated-bus-simulation>>), the Bus Simulation shall ensure that only the last <<low-cut-flexray-transmit-operation, `Transmit`>> operation within the time window is forwarded to the receiving FMU.
* In the case of directly connected Network FMUs (see <<common-concepts-direct-communication>>), the receiving FMU shall ensure that only the last <<low-cut-flexray-transmit-operation, `Transmit`>> operation is processed.
...
image::images/flexray_transmission_status.svg[width=800]
...
In the next step, the transmission is canceled via <<low-cut-flexray-cancel-operation, `Cancel`>> operation.
...
image::images/flexray_transmission_cancel.svg[width=500]
...
image::images/flexray_macroticks_segments_general.svg[width=700]
...
This behavior is defined differently depending on whether it is a static or dynamic segment.

What both segments have in common is that the Network FMU itself must know the appropriate time of a <<low-cut-flexray-transmit-operation, `Transmit`>> operation based on the FlexRay cycle and slot principle.

In concrete terms, this means that a Network FMU must provide a specified <<low-cut-flexray-transmit-operation, `Transmit`>> operation before the simulation reaches the associated slot’s action point (see `ActionPoint Offset` argument within the Configuration operation) and announce it via <<low-cut-tx-clocks, Tx Clocks>> mechanism.
...
That specifically means that the point in time for the start of the FlexRay cycle in nanoseconds can be computed within a Network FMU as
...
* stem:[L_{\mathrm{Cycle}}] defines the length of FlexRay cycle in nanoseconds (see `Macrotick Duration` and `Macroticks per Cycle` arguments within the <<low-cut-flexray-configuration-operation, `Configuration`>> operation).
...
The point in time for the start of the FlexRay cycle of the current iteration with focus to the simulation time can be computed within a Network FMU as
...
For a static segment, a Network FMU may provide multiple <<low-cut-flexray-transmit-operation, `Transmit`>> operations for the same slot/cycle during the simulation.
The operation provided closest to the slot’s action point (see `ActionPoint Offset` argument within the <<low-cut-flexray-configuration-operation, `Configuration`>> operation) shall be used.
Earlier operations for the same slot/cycle shall be overridden by later ones.
<<low-cut-flexray-transmit-operation, `Transmit`>> operations that are provided after the slot’s action point shall not be considered for the current time window and shall instead be used for the same slot and cycle in the next cycle iteration.
...
In the case of directly connected Network FMUs, the receiving FMU is responsible for processing the <<low-cut-flexray-transmit-operation, `Transmit`>> operation closest to the slot’s action point under consideration of the behavior defined by the <<low-cut-flexray-delivery-on-boundary-parameter, `DeliveryOnBoundary`>> configuration parameter.

.Bus Communication Points within static segment.
[#figure-flexray-static-segment-bus-communication-points]
image::images/flexray_static_segment_bus_communication_points.svg[width=900]

The starting point of the slot can be calculated at runtime based on the `FLEXRAY_CONFIG` configuration parameters as

[stem]
++++
    T_{\mathrm{Tx}_{\mathrm{Static}}}(i_{\mathrm{Iteration}}, i_{\mathrm{Cycle}}, i_{\mathrm{Slot}})
        = T_{\mathrm{CycleStart}}(i_{\mathrm{Iteration}}, i_{\mathrm{Cycle}})
...
The slot’s action point can be derived from this as follows:

[stem]
++++
    T_{\mathrm{AP}_{\mathrm{Static}}}(i_{\mathrm{iteration}}, i_{\mathrm{Cycle}}, i_{\mathrm{Slot}})
  = T_{\mathrm{Tx}_{\mathrm{Static}}}(i_{\mathrm{iteration}}, i_{\mathrm{Cycle}}, i_{\mathrm{Slot}}) + T_{\mathrm{ActionPoint}_{\mathrm{Static}}}
++++
...
[stem]
++++
    T_{\mathrm{Valid}_{\mathrm{Static}}}
        = \left(
            T_{\mathrm{AP}_{\mathrm{Static}}}(i_{\mathrm{iteration}} - 1, i_{\mathrm{Cycle}}, i_{\mathrm{Slot}}),
            \;
            T_{\mathrm{AP}_{\mathrm{Static}}}(i_{\mathrm{iteration}}, i_{\mathrm{Cycle}}, i_{\mathrm{Slot}})
        \right]
++++
...
* stem:[t \in T_{\mathrm{Valid}_{\mathrm{Static}}}
    \iff
    T_{\mathrm{AP}_{\mathrm{Static}}}(i_{\mathrm{iteration}} - 1, i_{\mathrm{Cycle}}, i_{\mathrm{Slot}})
    < t \le
    T_{\mathrm{AP}_{\mathrm{Static}}}(i_{\mathrm{iteration}}, i_{\mathrm{Cycle}}, i_{\mathrm{Slot}})].

A <<low-cut-flexray-transmit-operation, `Transmit`>> operation is valid if it is provided at a time stem:[t] within the interval spanning from immediately after the action point of slot stem:[i_{\mathrm{Slot}}] in stem:[i_{\mathrm{Cycle}}] of iteration stem:[i_{\mathrm{iteration}} - 1] to the action point of the same slot and cycle in iteration stem:[i_{\mathrm{iteration}}].

<<figure-flexray-transmission-static-segment>> illustrates a sequence of operations focusing on the respective actors and corresponding FlexRay slots.
...
image::images/flexray_transmission_static_segment.svg[width=900]

====== Dynamic Segment [[low-cut-flexray-dynamic-segment]]

Within the dynamic segment, the usage of <<low-cut-flexray-transmit-operation, `Transmit`>> operations is analogous to the static segment.
Network FMUs need to provide <<low-cut-flexray-transmit-operation, `Transmit`>> operations within a well-defined time window and with respect to the designated minislot.
As in the static segment, the Last-Is-Best principle applies and <<low-cut-flexray-transmit-operation, `Transmit`>> operations are allowed from a `Minislot ActionPoint` (see <<low-cut-flexray-configuration-operation, `Configuration`>> operation) to the same point in the next cycle iteration.
The <<low-cut-flexray-transmit-operation, `Transmit`>> operation closest to the slot’s minislot action point is used.
...
For directly connected Network FMUs, the Last-Is-Best principle shall be enforced by the receiving FMU and the <<low-cut-flexray-delivery-on-boundary-parameter, `DeliveryOnBoundary`>> configuration parameter must be taken into account in order to process the <<low-cut-flexray-transmit-operation, `Transmit`>> operation at the correct time.

.Bus Communication Points within dynamic segment.
[#figure-flexray-dynamic-segment-bus-communication-points]
image::images/flexray_dynamic_segment_bus_communication_points.svg[width=900]

Within a Network FMU the start time of the last valid minislot for providing a specified <<low-cut-flexray-transmit-operation, `Transmit`>> operation, with respect to the specified `Slot ID`, can be computed as

[stem]
++++
T_{\mathrm{Tx}_{\mathrm{Dynamic}}}(i_{\mathrm{iteration}}, i_{\mathrm{Cycle}}, i_{\mathrm{Slot}})
    =
    T_{\mathrm{Tx}_{\mathrm{Static}}}(i_{\mathrm{iteration}}, i_{\mathrm{Cycle}}, N_{\mathrm{StaticSlot}} + 1)
...
NOTE: It should be noted that stem:[T_{\mathrm{Start}}] is already taken into account in stem:[T_{\mathrm{Tx}_{\mathrm{Static}}}(...)] and doesn't need to be considered a second time.

The minislot’s action point can be derived from this as follows:

[stem]
++++
T_{\mathrm{AP}_{\mathrm{Dynamic}}}(i_{\mathrm{iteration}}, i_{\mathrm{Cycle}}, i_{\mathrm{Slot}})
    = T_{\mathrm{Tx}_{\mathrm{Dynamic}}}(i_{\mathrm{iteration}}, i_{\mathrm{Cycle}}, i_{\mathrm{Slot}})
...
[stem]
++++
T_{\mathrm{Valid}_{\mathrm{Dynamic}}}
    = \left(
        T_{\mathrm{AP}_{\mathrm{Dynamic}}}(i_{\mathrm{iteration}} - 1, i_{\mathrm{Cycle}}, i_{\mathrm{Slot}}),
        \;
       T_{\mathrm{AP}_{\mathrm{Dynamic}}}(i_{\mathrm{iteration}}, i_{\mathrm{Cycle}}, i_{\mathrm{Slot}})
    \right]
++++
...
* stem:[t \in T_{\mathrm{Valid}_{\mathrm{Dynamic}}}
\iff
T_{\mathrm{AP}_{\mathrm{Dynamic}}}(i_{\mathrm{iteration}} - 1, i_{\mathrm{Cycle}}, i_{\mathrm{Slot}})
< t \le
T_{\mathrm{AP}_{\mathrm{Dynamic}}}(i_{\mathrm{iteration}}, i_{\mathrm{Cycle}}, i_{\mathrm{Slot}})].
...
image::images/flexray_transmission_dynamic_segment.svg[width=900]

If no Bus Simulation is involved, the receiving FMU shall ensure that incoming <<low-cut-flexray-transmit-operation, `Transmit`>> operations are processed in the correct minislot, considering the transmission duration and the number of remaining minislots within the current cycle.
<<low-cut-flexray-transmit-operation, `Transmit`>> operations whose transmission duration exceeds the number of remaining minislots in the current cycle shall be deferred and processed in the same slot and cycle in the next cycle iteration.
...
image::images/flexray_error_handling_overview.svg[width=800]
