...
If multiple <<low-cut-flexray-symbol-operation, `Symbol`>> operations with `Type` = `COLLISION_AVOIDANCE_SYMBOL` are received at the same time, the Bus Simulation shall solve this conflict based on the smallest Slot ID.
If no Bus Simulation is involved, the Network FMUs shall solve this conflict based on the smallest Slot ID.
