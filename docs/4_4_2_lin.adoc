...
All three LIN cases, in detail a LIN Header, a LIN Response and a combination of a Header and Response are considered here.
...
| Checksum Type | 1 byte | Indicates the checksum type of the specified LIN frame (see <<#table-lin-checksum-types-values>>).
...
h|Behavior
3+|The <<low-cut-lin-transmit-operation, `Transmit`>> operation shall be provided by Network FMUs to initiate the transmission of a LIN frame that contains the specified Header, Response or a combination of both cases.
...
|HEADER RESPONSE
|0x03
|Indicates a combination of a LIN Header and a LIN Response, both from LIN Master. 
In this case the `Data Length` argument of a <<low-cut-lin-transmit-operation, `Transmit`>> operation contains the length of the `Data` argument value.
If a combined Header and Response from the LIN Master is to be simulated, this `Frame Type` must always be used, and not a consecutive combination of two <<low-cut-lin-transmit-operation, `Transmit`>> operations where `Frame Type` is set to `Header` and `Response`.
...
|PID
|1 byte
|The protected ID of the LIN message that was transmitted while the error happened.
...
The error can occur during a collision of multiple LIN Response frames when Event-Triggered-Frames are used.
...
Within this situation, no valid `PID` can be used as an argument value for the respective <<low-cut-lin-bus-error-operation, `Bus Error`>> operation and shall be set to zero.
...
|BAUDRATE|0x01|This code indicates that a LIN baud rate should be configured for the Bus Simulation.
|NODE_DEFINITION|0x02|This code configures the specified node type (Master or Slave) within a Bus Simulation.
...
The communication process of a LIN bus is based on a simple master-slave principle and is strictly time-controlled.
The LIN Master assumes complete control over the bus and initiates all messages.
Every communication begins with a so-called LIN Header, which the LIN Master sends.
After the LIN Header has been sent, the LIN Response phase follows.
In this phase, either the addressed LIN Slave or the LIN Master itself sends the actual data.
...
A <<low-cut-lin-confirmation-operation, `Confirm-`>> and <<low-cut-lin-bus-error-operation, `Bus Error`>> operation represents feedback from a Bus Simulation for a previously carried out <<low-cut-lin-transmit-operation, `Transmit`>> operation.
Depending on the <<low-cut-lin-bus-notification-parameter, `BusNotifications`>> parameter, for a Network FMU a successful transmission of a <<low-cut-lin-transmit-operation, `Transmit`>> operation results in a <<low-cut-lin-confirmation-operation, `Confirm`>> operation, an unsuccessful <<low-cut-lin-transmit-operation, `Transmit`>> operation in a <<low-cut-lin-bus-error-operation, `Bus Error`>> operation.
...
In this scenario FMU 2 does not requires internally a self-confirmation and leave this out.

.Direct Confirmation of transmitted network data.
[#figure-lin-direct-communication]
image::lin_direct_confirmation.svg[width=40%, align="center"]
...
The following <<#figure-lin-confirmation-with-bus-simulation-fmu>> illustrates the behavior, whereby FMU 1 transmits a LIN Header to FMU 2 via a Bus Simulation by using a <<low-cut-lin-transmit-operation, `Transmit`>> operation.
After this the Bus Simulation confirms that transmission via <<low-cut-lin-confirmation-operation, `Confirm`>> operation.
...
Additionally a <<low-cut-lin-confirmation-operation, `Confirm`>> operation is given to FMU 2 from the Bus Simulation.

.Confirmation of transmitted network data via Bus Simulation.
[#figure-lin-confirmation-with-bus-simulation-fmu]
image::lin_confirmation_with_bus_simulation_fmu.svg[width=70%, align="center"]
...
The second variant describes the collision of LIN frames during the response phase.
...
<<#figure-lin-collision-with-bus-simulation-fmu>> illustrates a LIN Master (FMU 1) providing a <<low-cut-lin-transmit-operation, `Transmit`>> operation via Bus Simulation to two LIN Slaves (FMU 2 and FMU 3).
Each LIN Slave responds with its own <<low-cut-lin-transmit-operation, `Transmit`>> operation.
...
image::lin_collision_with_bus_simulation_fmu.svg[width=90%, align="center"]
...
This baud rate information can be configured by using `BAUDRATE` configuration kind of the <<low-cut-lin-configuration-operation, `Configuration`>> operation.
...
By using the `NODE_DEFINITION` kind of <<low-cut-lin-configuration-operation, `Configuration`>> operation, the specified node type `LIN_MASTER` or `LIN_SLAVE` needs to be adjusted.
