...
h|Description
3+|Represents transmission of either a LIN Frame Header or LIN Frame Response.
...
|Frame Part
|1 byte
|Indicates the type of the specified LIN frame (see <<#table-lin-frame-part-values>>).
...
Allowed values for the `Data Length` depend on the value of `Frame Part` and are specified in <<#table-lin-frame-part-values>>.
...
h|Behavior
3+|The <<low-cut-lin-transmit-operation, `Transmit`>> operation shall be provided by Network FMUs to indicate the transmission of either a LIN Header or a LIN Response to a corresponding LIN Header.
...
The following values for `Frame Part` argument are specified:

.Overview of the available frame part values.
[#table-lin-frame-part-values]
[cols="2,1,5"]
|====
h|Frame Part h|Value h|Description
...
This <<low-cut-lin-bus-error-operation, `Bus Error`>> operation is a direct reaction to a <<low-cut-lin-transmit-operation, `Transmit`>> operation (`Frame Part` is set to `HEADER`) and shall be delivered to the Network FMU that is configured as <<table-lin-configuration-node-definition-kinds, `LIN_COMMANDER`>> within the specified LIN network.

|CHECKSUM_ERROR
|0x02
|A `CHECKSUM_ERROR` error can occur during a collision of LIN Responses when Event-Triggered-Frames are used.
This <<low-cut-lin-bus-error-operation, `Bus Error`>> operation is a direct reaction to a <<low-cut-lin-transmit-operation, `Transmit`>> operation (`Frame Part` is set to `RESPONSE`) and shall be delivered to the Network FMU that is configured as <<table-lin-configuration-node-definition-kinds, `LIN_COMMANDER`>> of the specified LIN network.

|IDENTIFIER_PARITY_ERROR
|0x03
|A `IDENTIFIER_PARITY_ERROR` error can occur during a collision of LIN Headers.
This <<low-cut-lin-bus-error-operation, `Bus Error`>> operation is a direct reaction to a <<low-cut-lin-transmit-operation, `Transmit`>> operation (`Frame Part` is set to `HEADER`) and shall be delivered to all Network FMUs of the specified LIN network.
...
The configuration of the `NODE_TYPE` parameter must be completed by all Network FMUs before any <<low-cut-lin-transmit-operation, `Transmit`>> operation is produced.
...
Within the <<low-cut-lin-transmit-operation, `Transmit`>> operation, the different frame parts are qualified via the `Frame Part` argument.
...
image::images/lin_direct_confirmation.svg[width=60%]
...
image::images/lin_confirmation_with_bus_simulation_fmu.svg[width=70%]
...
image::images/lin_collision_with_bus_simulation_fmu.svg[width=90%]
...
By using the `NODE_TYPE` configuration parameter of <<low-cut-lin-configuration-operation, `Configuration`>> operation, the specified node type `LIN_COMMANDER` or `LIN_RESPONDER` needs to be adjusted.
...
image::images/lin_error_handling_overview.svg[width=80%]
...
[#figure-lin-wake-up]
image::images/lin_wake_up.svg[width=70%]
