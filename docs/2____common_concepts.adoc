...
If triggered output clocks are used, the importer must take care to schedule and potentially roll-back FMUs that have advanced their `doStep` past such a (surprising) triggered clock activation from another FMU.
