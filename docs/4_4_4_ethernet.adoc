...
Depending on the <<low-cut-can-bus-notification-parameter, `BusNotifications`>> parameter the Bus Simulation confirms the transmission with a <<low-cut-ethernet-confirm-operation, `Confirm`>> operation.

<<#figure-ethernet-transmit>> illustrates the behavior, whereby FMU 1 transmits network data to FMU 2 via a Bus Simulation under the condition that <<low-cut-can-bus-notification-parameter, `BusNotifications`>> parameter is `false`.
...
<<#figure-ethernet-transmit-with-confirm>> illustrates the behavior, whereby FMU 1 transmits network data to FMU 2 via a Bus Simulation under the condition that <<low-cut-can-bus-notification-parameter, `BusNotifications`>> parameter is set to `fmi3True`.
