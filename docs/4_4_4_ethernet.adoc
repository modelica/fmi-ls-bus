...
For creating simulation systems with the different kind of Ethernet connections, Bus Terminals can either be grouped by a <<low-cut-ethernet-segment-terminal, Ethernet Segment Terminal>> or <<low-cut-ethernet-switch-terminal, Ethernet Switch Terminal>>. 
Since an Ethernet switch can be implemented both in the Network FMU and in the Bus Simulation, a Ethernet Switch Terminal can be used for both.
However, a Segment Terminal should only be used to group Bus Terminals of a Bus Simulation. 

.Ethernet topologies.
image::ethernet_topology.svg[width=100%, align="center"]
...
====== Ethernet Segment Terminal [[low-cut-ethernet-segment-grouping-terminal]]
...
Bus Terminals which are used only for routing Ethernet franmes, may be grouped by this terminal.
The <<low-cut-grouping-terminals, Grouping Terminal>> additionally defines the following properties:
