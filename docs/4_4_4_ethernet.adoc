...
[cols="17,1,8,1,5,13,13,1,2,5,5,2"]
|====
.2+h|Operation Name
...
image::images/ethernet_topology_switch_inside_Bus_Simulation.svg[width=700]
...
image::images/ethernet_topology_switch_inside_FMU.svg[width=700]
...
NOTE: The default value `false` allows a simple integration of Network FMUs to simulation scenarios where <<low-cut-ethernet-confirm-operation, `Confirm`>> or <<low-cut-ethernet-bus-error-operation, `Bus Error`>> operations are not used.
...
image::images/ethernet_transmit.svg[width=700]
...
image::images/ethernet_transmit_with_confirm.svg[width=700]
...
NOTE: Note: In this scenario the ongoing transmission continues.
...
image::images/ethernet_transmit_collision_avoided.svg[width=900]
...
NOTE: Note: In this scenario the ongoing transmission is destroyed as well.
...
image::images/ethernet_transmit_collision.svg[width=900]

In this context, stem:[T_{\mathrm{transmit}}] defines the length of the message transmission.

====== Fragmented Transmission [[low-cut-ethernet-fragmented-transmission-using-merge-sublayer]]
...
image::images/ethernet_transmit_fragmented.svg[width=800]
...
image::images/ethernet_error_handling_overview.svg[width=800]
...
The Ethernet-specific wake-up pulse can be simulated by using the <<low-cut-ethernet-wakeup-operation, `Wakeup`>> operation, initiated by one Network FMU.
...
image::images/ethernet_wake_up.svg[width=700]
