...
For the definition of point-to-point connections or multidrop segments, a Bus Simulation may subgroup the respective Bus Terminals either by a <<low-cut-ethernet-point-to-point-grouping-terminal,Point-to-Point Grouping Terminal>> or a <<low-cut-ethernet-multidrop-grouping-terminal, Multidrop Grouping Terminal>>.
...
===== Grouping Terminals [[low-cut-ethernet-grouping-terminal-definition]]
This section defines special <<low-cut-grouping-of-bus-terminals, groups of Bus Terminals>> for Ethernet.
The usage is optional but recommended.

====== Point-to-Point Grouping Terminal [[low-cut-ethernet-point-to-point-grouping-terminal]]
Bus Terminals which are connected point-to-point may be grouped by this terminal.
The <<low-cut-grouping-terminals, Grouping Terminal>> additionally defines the following properties:

Attributes of a Point-to-Point Terminal::
 * `terminalKind` must be `org.fmi-ls-bus.ethernet-point-to-point-terminal`.

====== Multidrop Grouping Terminal [[low-cut-ethernet-multidrop-grouping-terminal]]
Bus Terminals which are connected multidroped may be grouped by this terminal.
The <<low-cut-grouping-terminals, Grouping Terminal>> additionally defines the following properties:
