...
The `causality` for the Tx_Data variable shall be `output`, but for the Tx_Clock variable it depends of the selected clock type (see <<low-cut-tx-time-based-clock-variables, Tx Clock Variables>>).  
...
In consequence, bus communication points and regular communication points coincide and takes place at the same time.
