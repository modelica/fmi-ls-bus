...
===== Grouping Terminals [[low-cut-grouping-terminals, Grouping Terminals]]
...
Depending on the bus-specific definition, buses can introduce special terminal kinds for Grouping Terminals to give them more semantics and to use their functionality.

Attributes of the Grouping Terminal::
 * `terminalKind` may be explicitly defined by a specific bus to give more semantic to a respective Grouping Terminal; otherwise not set.
 * `matchingRule` must be `none`.

Elements of a Grouping Terminal::
 * A Grouping Terminal shall contain no `<TerminalMemberVariable>` elements
