...
While <<low-cut-tx-clocks, Tx Clock>> and <<low-cut-rx-clocks, Rx Clock>>) are used for scheduling and time synchronization,  <<low-cut-tx-rx-data-variables, Tx/Rx Data Variables>>) are used to carry the layered standard bus protocol operations in binary form.
...
Although the `causality` of a `time-based` `Tx_Clock` and a `triggered` `Rx_Clock` are each `input`, it is allowed to connect both, since a `time-based` Clock of an FMU is considered only to be a request to the importer to provide an corresponding Clock source.
...
_[Note: For clarity always a connecting line between `Tx_Clock` and `Rx_Clock` is shown in figures of this document.]_
