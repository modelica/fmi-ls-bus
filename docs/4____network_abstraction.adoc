...
<<low-cut-communication-points>> illustrates the operating principle of the exchange of Bus Operations using a `Tx_Clock` of type `countdown` Clock.
...
Within the first communication point (black dot), the time is announced when FMU 1 wants to transmit data via the Bus Simulation to FMU 2 by configuring the `countdown` `Tx_Clock`.
In the next step, the simulation system stops at a Bus Communication Point, so when the configured time is reached (red dot).
The Bus Operation is then passed from FMU 1 to Bus Simulation.
Bus Simulation itself also uses a `countdown` `Tx_Clock` and sets a point in the future at which the Bus Operation should be transmitted to FMU 2.
Using these configured time periods, the actual transmission time is simulated.
In the last step, the Bus Operation arrives at FMU 2.
...
_[Selecting a suitable `intervalVariability` for `Tx Clock' allows the FMU (or better the exporting tool) to balance the accuracy and performance of its network communication:_
...
_- Using `periodic` Clocks and queueing the operations to be transmitted reduces the number of `Event Mode` entries and might speeds up the simulation at the cost of simulation accuracy._
