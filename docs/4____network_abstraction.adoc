...
Exchanging data in terms of the <<Overview-Layered-Standard-Bus-Protocol, Layered Standard Bus Protocol>> via variables grouped by a dedicated bus terminal (see <<ClockVariableLowCut>>), allows the implementation of bus simulations in a wide range from idealized up to detailed behavior, including e.g., timing, arbitration, error, status and other effects.
...
The point in time at which FMUs will (potentially) send new network messages must be given to the importer in advance.
...
The Tx/Rx variables are of type `fmi3Binary` and contain one or more bus operations, as sent or receive by the FMU.
 ...
 * `name` is the network name, e.g., `Powertrain`, see <<ExampleLowCut, Example>> and constraints.
...
For each supported bus type, the <<Overview-Layered-Standard-Bus-Protocol, Layered Standard Bus Protocol>> is defined.
This chapter describes the operations of the <<Overview-Layered-Standard-Bus-Protocol, Layered Standard Bus Protocol>> to enable the simulation of supported bus types.
