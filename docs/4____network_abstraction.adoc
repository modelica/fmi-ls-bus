...
Specific bus systems can add their own configuration parameters within a Bus Terminal.
 ...
 * `terminalKind` of a sub `<Terminal>` for additional configuration parameters must be set to `org.fmi-ls-bus.network-terminal.configuration`.
 * `matchingRule` of a sub `<Terminal>` for additional configuration parameters must be set to `bus`.
 * `name` of a sub `<Terminal>` for additional configuration parameters must be set to `Configuration`.

Element definitions::
 * Every Bus Terminal contains at least four variables each in the role of: `Tx_Data`, `Tx_Clock`, `Rx_Data` or `Rx_Clock`.
 ...
 * The `<Terminal>` for additional configuration parameters is nested within the actual Bus Terminal.
 * The `<TerminalMemberVariable>.memberName` of a configuration parameter is fixed and defines it's functionality.
 * `<TerminalMemberVariable>.variableKind` is `signal` within both `<Terminal>` kinds.
