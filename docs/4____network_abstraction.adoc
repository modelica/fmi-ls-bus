...
|`periodic`|`constant`|_The Bus Communication Point interval is defined in the `modelDescription.xml`._
|`periodic`|`fixed`|_The Bus Communication Point interval is defined in the `modelDescription.xml` or must be provided in `Initialization Mode`._
|`periodic`|`tunable`|_The Bus Communication Point interval is defined in the `modelDescription.xml` or must be provided in `Initialization Mode` and can be changed in `Event Mode`._
|`aperiodic`|`changing`|_The Bus Communication Point interval must be provided in `Initialization Mode` and can be changed in `Event Mode` if this Clock ticked._
|`aperiodic`|`countdown`|_The Bus Communication Point interval must be provided in `Initialization Mode` and can be changed in every `Event Mode`._
|====
