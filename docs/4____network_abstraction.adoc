...
Exchanging data in terms of the <<Overview-Layered-Standard-Bus-Protocol, Layered Standard Bus Protocol>> via variables grouped by a dedicated bus terminal (see <<ClockVariableLowCut>>), allows the implementation of bus simulations in a wide range from idealized up to a detailed behavior including e.g. timing, arbitration, error, status and other effects.
During simulation, the network communication is controlled by send and receive bus operations.
Based on these operations, Network FMUs have to react as specified by this layered standard in dependency of their role.
This role can be a Bus Simulation FMU or an importer with bus simulation support on one side, and Network FMUs (e.g. virtual ECUs) on the other side (see <<SystemCompositions, System Compositions>>).
...
Depending on the <<SystemCompositions, System Composition>>, the importer forwards the Transmit operation either directly to receiving FMUs or to a dedicated bus simulation.
...
Compositions with fixed-step size and variable-step size FMUs requires the decoupling of the Bus Simulation logic and the Bus Simulation timing.
...
For this operation applies: `Total Length = 7 + <Arg2_Length>`.
...
3+|NOOP
h|Description
3+|Represents no pending operations.
h|OP Code [hex]
3+|0x00
.3+h|Content
h|Argument
...
|OP Code
|1 byte
|Contains the OP Code (0x00) of the operation.
...
h|Behavior
3+|The specified operation can be produced by Network FMUs and the Bus Simulation.
Networked FMU shall ignore received NOOP operations.
FMU Importer can decide not to transmit the NOOP operation to Networked FMUs.

|====

==== Format Error [[Format-Error-Operation]]
...
|4 byte
|Defines the cumulative length of all arguments in bytes.
For this operation applies: `Total Length = 5 + Length of Data argument in bytes`.
...
For each supported bus types the <<Overview-Layered-Standard-Bus-Protocol, Layered Standard Bus Protocol>> is defined.
This chapter basically describes the operations of the <<Overview-Layered-Standard-Bus-Protocol, Layered Standard Bus Protocol>> to enable the simulation of supported bus types.
