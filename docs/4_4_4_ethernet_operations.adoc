...
The following applies for this operation: `Length = 26 + n`.


3+| Start delimiter
| 1 byte
| The start frame (SFD) or start mPacket (SMD) delimiter. Always set to 0xD5 for ordinary Ethernet frames when not using preemption/fragmentation.

3+| Fragment counter
| 1 byte
| Fragment counter of the continuation mPacket. Must be set to 0, if this is not a continuation mPacket.

3+| Last fragment
| 1 byte
| Indicates whether this is the last fragment of a fragmented Ethernet frame. Must be set to TRUE if this is a full Ethernet frame.

3+| Destination address
| 6 bytes
| The destination address of the frame. Only applicable if this is a full Ethernet frame or the first fragment of an Ethernet frame.
...
h|Description 5+| Base page for section 28 auto-negotiation.
...
h|Description 5+| Base page for section 98 auto-negotiation.
...
[cols="6,1,5,4,3,20"]
|====
h|Name 5+| Auto Negotiation Page
h|Description 5+| Next page for section 28 or 98 auto-negotiation.
...
3+| Extended page
| 1 byte
| Indicates that this is an extended page (section 28). Must always be TRUE when using section 98 auto-negotation.
...
3+| Toggle
| 1 byte
| Toggle bit for synchronization. Reserved. Must be FALSE.

3+| Ack
| 1 byte
| Acknowledgement. Reserved. Must be FALSE.

3+| Ack2
| 1 byte
| Acknowledgement 2. Reserved. Must be FALSE.
