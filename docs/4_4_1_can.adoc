...
As soon as an arbitration is lost, an <<low-cut-can-arbitration-lost-operation, `Arbitration Lost`>> operation shall be returned to the respective sender within the same `Event Mode`, the specified <<low-cut-can-transmit-operation, `Transmit`>>- and <<low-cut-can-confirm-operation, `Confirm`>> operations are provided by the specified Bus Simulation.
_As a recommendation, at least one `Step Mode` shall be between that actions._
