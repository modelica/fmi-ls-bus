...
Otherwise, the <<TransmitOpCodeCanLowCut, Transmit operation>> shall be discard and an <<ArbitrationLostOpCodeCanLowCut, Arbitration Lost operation>> shall be sent to the Network FMU (see <<CanArbitration>>).
...
Because the Bus Simulation shall choose a default behavior, it may be useful in several scenarios that Network FMUs finnish configuration before the production of <<TransmitOpCodeCanLowCut, Transmit operations>>.
...
Once this option deactivated, the Bus Simulation buffers the frame after losing arbitration and sends it as soon as possible.
