...
Using configuration parameters on Bus Terminal level, FMUs can be parameterized according to importer specifications.
...
CAN Bus Terminals of a Bus Simulation (FMU) may include this configuration parameter, by using a `calculatedParameter` instead of a `parameter` to participate in an importer parameter propagation feature.
