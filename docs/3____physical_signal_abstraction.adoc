...
Here the signals (_Signal 1...8_) from PDUs, which in turn are assigned to frames, are modeled as clocked FMU variables of a specific type.
 ...
 * There must be one `<Terminal>` element per network frame described in the <<high-cut-network-description-files>>, if their signal and lock variables are present in the `modelDescription.xml`.

Annotation element::
 * There must be an `<Annotation>` element defining which node or nodes (as comma-separated list without spaces) of the <<high-cut-network-description-files>> are wrapped inside the FMU.
